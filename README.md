...
![DEX 费率对比系统界面](screenshot-20260318-151638.png)
...
2.  **配置文件** (`config.py`)：
    - 程序会自动检测配置是否有效（是否包含 "YOUR_API_KEY"）。
    - **智能兜底**：如果检测到配置无效（用户未修改），系统会自动切换到 `DEFAULT_ARBITRUM_RPC` 指定的默认公共节点，确保证程序能直接运行。
3.  **默认公用节点**：如果以上均未配置，使用内置默认公共节点。

> **修复日志 (2026-01-25)**:
>
> 1. **数据自动纠错**：修复了 Hyperliquid 长期运行时因交易所上币/调整顺序导致的数据错位（价格卡死）问题。现在系统会每分钟自动校准资产列表。
> 2. **配置加载增强**：修复了打包版本 (.exe) 无法读取外部 `config.py` 的问题。现在支持在 exe 旁边放置配置文件。
...
- **配置调整**:
  - `GOLD` 预期收敛价差调整为 **$4**
  - `SILVER` 预期收敛价差调整为 **$0.3**
  - `XYZ100` (纳指) 预期收敛价差调整为 **$20**

### 2026-01-26

- 🔄 **WebSocket 自动重连**：
  - 为 Hyperliquid WebSocket 客户端添加智能重连机制
  - 支持断线后自动重连，使用指数退避策略（1秒 → 2秒 → 4秒 → ... → 60秒）
  - 连接成功后自动重置重连延迟，确保长时间运行稳定性
  - 完善的连接状态日志输出，便于监控和排查问题

### 2026-01-25

- 🐛 **严重 BUG 修复**：
  - 修复 Hyperliquid 长时间运行后价格卡死/错位的问题（根本原因：交易所资产列表动态变化）。
  - 新增每分钟自动校准机制，确保数据永久准确。
- ⚙️ **配置系统升级**：
  - 修复打包版无法读取外部 `config.py` 的路径问题。
  - 新增 `DEFAULT_ARBITRUM_RPC` 兜底配置。
  - 新增智能配置校验：当检测到默认 API Key 时自动回退到公共节点，避免程序崩溃。
...
- **配置更新**:
  - `GOLD` 预期收敛价差调整为 **$2**
  - `SILVER` 预期收敛价差调整为 **$0.1**
  - `CL` (原油) 预期收敛价差调整为 **$0.06**
  - 新增 `CL` (原油) 到优先显示列表和监控列表
- **性能优化**:
  - Ostium 轮询间隔缩短为 **1秒**，提升数据实时性
